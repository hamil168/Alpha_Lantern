...
def user_classification(user_input, model):
    """
        ...
        Output a is a string of 0's and 1's,
        and a list of predictions, each between 0.0 and 1.0
        and a list of classes, matching the string but broken into list elements
        (its a bit clumsy but the infromation will all be there for a time crunch)
    ...
    for label in y_pred:
        ...
        classes = list(map(lambda x: ceil(x - CLASSIFICATION_THRESHOLD), label))
        y_class.append(classes)

    return yp, y_pred, y_class
